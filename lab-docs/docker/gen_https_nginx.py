...
Variacoes:
  ...
  --https           gera CA local + cert (SAN) e escuta na 443 (precisa de openssl)
...
"""
import sys, os, string, argparse, urllib.request, subprocess, textwrap
...
ap.add_argument("--https", action="store_true",
                help="Habilita HTTPS: gera uma CA local + cert (SAN) e escuta na 443.")
ap.add_argument("--porta-https", type=int, default=443, help="Porta do host para o HTTPS")
args = ap.parse_args()
...
HTTPS, PORTA_HTTPS = args.https, args.porta_https
...
# ---------------- certificados TLS (CA local + cert com SAN) ----------------
# Gera uma CA local e um certificado assinado por ela cobrindo lab + *.<grupo>.lab.
# Clientes instalam o ca.crt UMA vez e todos os *.lab ficam com HTTPS valido.
def gerar_certificados():
    certs = f"{BASE}/certs"
    os.makedirs(certs, exist_ok=True)
    ca_key, ca_crt = f"{certs}/ca.key", f"{certs}/ca.crt"
    srv_key, srv_crt = f"{certs}/server.key", f"{certs}/server.crt"
    csr, cnf = f"{certs}/server.csr", f"{certs}/san.cnf"

    # SAN: portal (lab) + wildcard por grupo (*.n21-a.lab cobre n8n.n21-a.lab, etc.)
    sans = [DOMINIO] + [f"*.{g}.{DOMINIO}" for g in todos]
    alt = "\n".join(f"DNS.{i+1} = {d}" for i, d in enumerate(sans))
    ...
    try:
        # Reaproveita a CA se ja existir (evita reinstalar o ca.crt nos clientes)
        if not (os.path.exists(ca_key) and os.path.exists(ca_crt)):
            ...
            run(["openssl", "req", "-x509", "-new", "-nodes", "-key", ca_key,
                 ...
                 "-subj", f"/CN=Lab Root CA ({DOMINIO})/O=Lab {TURMA}"])
            ca_status = "CA nova gerada"
        else:
            ca_status = "CA existente reaproveitada"
        # Sempre (re)gera o cert do servidor (SAN pode ter mudado)
        run(["openssl", "genrsa", "-out", srv_key, "2048"])
        run(["openssl", "req", "-new", "-key", srv_key, "-out", csr, "-config", cnf])
        run(["openssl", "x509", "-req", "-in", csr, "-CA", ca_crt, "-CAkey", ca_key,
             "-CAcreateserial", "-out", srv_crt, "-days", "3650", "-sha256",
             "-extfile", cnf, "-extensions", "v3_req"])
    except FileNotFoundError:
        sys.exit("ERRO: --https precisa do 'openssl' instalado no sistema.")
    except subprocess.CalledProcessError as e:
        sys.exit(f"ERRO ao gerar certificados com openssl: {e}")

    # Instrucoes de instalacao da CA nos clientes
    with open(f"{certs}/INSTALL-CA.txt", "w", encoding="utf-8") as f:
        f.write(textwrap.dedent(f"""\
            Instalar a CA do lab (ca.crt) — faca UMA vez por cliente.
            Depois disso, https://<qualquer>.{DOMINIO}/ fica com cadeado valido.

            Windows (todas as contas da maquina):
              certutil -addstore -f "ROOT" ca.crt
              (ou 2x clique no ca.crt > Instalar > Maquina Local >
               "Autoridades de Certificacao Raiz Confiaveis")
              Obs.: o Firefox usa loja propria (Configuracoes > Certificados > Importar).

            Linux (Debian/Ubuntu):
              sudo cp ca.crt /usr/local/share/ca-certificates/lab-{DOMINIO}.crt
            ...
            SANs cobertos por este certificado:
            """) + "\n".join(f"  - {d}" for d in sans) + "\n")
    return ca_status

CERTS_MSG = ""
if HTTPS:
    CERTS_MSG = gerar_certificados()


# ---------------- docker-compose.yml ----------------
nginx_ports = f'      - "{PORTA}:80"\n'
nginx_vols = (
    "      - ./nginx.conf:/etc/nginx/nginx.conf:ro\n"
    "      - ./html:/usr/share/nginx/html:ro\n"
)
if HTTPS:
    nginx_ports += f'      - "{PORTA_HTTPS}:443"\n'
    nginx_vols += "      - ./certs:/etc/nginx/certs:ro\n"
...
if HTTPS:
    ssl_http_block = (
        "\n    # ---- TLS (cert assinado pela CA local, cobre lab + *.<grupo>.lab) ----\n"
        "    ssl_certificate     /etc/nginx/certs/server.crt;\n"
        "    ssl_certificate_key /etc/nginx/certs/server.key;\n"
        "    ssl_protocols       TLSv1.2 TLSv1.3;\n"
    )
# linhas 'listen' extras para 443 (portal recebe default_server)
listen_portal_443 = "        listen 443 ssl default_server;\n" if HTTPS else ""
listen_host_443 = "        listen 443 ssl;\n" if HTTPS else ""
...
for g in todos:
    ...
    for s in SERVICOS:
        ...
        cards.append(
            ...
            f'                </div>\n'
            f'                <a href="//{host}/" target="_blank" rel="noopener"\n'
...
<script>
  ...
  hosts.forEach(function(h){
    fetch(location.protocol+"//"+h+"/ping",{cache:"no-store"})
...
if HTTPS:
    print(f"  - {BASE}/certs/              (ca.crt, server.crt/key, INSTALL-CA.txt)")
if SEM_DNS:
...
if HTTPS:
    print(f"HTTPS: ligado (porta {PORTA_HTTPS}) — {CERTS_MSG}.")
print("\nComo usar:")
passo = 1
if PORTA == 80:
...
passo += 1
if HTTPS:
    print(f"  {passo}) Nos CLIENTES, instale a CA {BASE}/certs/ca.crt (uma vez):")
    print(f"       Windows: certutil -addstore -f \"ROOT\" ca.crt")
    print(f"       Linux:   sudo cp ca.crt /usr/local/share/ca-certificates/lab.crt && sudo update-ca-certificates")
    print(f"       (detalhes em {BASE}/certs/INSTALL-CA.txt)")
    passo += 1
proto = "https" if HTTPS else "http"
porta_ativa = PORTA_HTTPS if HTTPS else PORTA
porta_padrao = 443 if HTTPS else 80
sufixo = "" if porta_ativa == porta_padrao else f":{porta_ativa}"
print(f"  {passo}) Abra {proto}://{DOMINIO}{sufixo}/  e veja o auto-teste.")
passo += 1
